...
This extension board, as shown in figure \ref{fig:ethernetboard}, can be used for sending and transmitting IEEE802.3 frames, and therefore embedding the developmentboard into a network. The board features an ethernet controller ENC28J60 that exchanges data with a microcontroller via standard SPI. The board can be connected to dsPIC, AVR-8051 and PIC by three 10-pin IDC connectors. The board needs 3.3V voltage supply, but has a 74LVC1T45 bus transceiver chip for running the board with 5V. Therefore, jumper J1 has to be set correctly.
